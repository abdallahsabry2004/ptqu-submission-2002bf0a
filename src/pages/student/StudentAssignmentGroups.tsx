...
const StudentAssignmentGroups = () => {
  ...
  const load = async () => {
    ...
    const { data: gs } = await supabase
      ...
      .select("id, name, max_size, created_by, assignment_group_members(student_id)")
  ...
  const createGroup = async () => {
    ...
    // auto-join
    const { error: e2 } = await supabase.from("assignment_group_members").insert({
      group_id: (g as any).id,
      student_id: user!.id,
      assignment_id: assignmentId!,
    });
    if (e2) toast.error(e2.message);
    else toast.success("تم إنشاء المجموعة والانضمام إليها");
    setOpenCreate(false);
    setNewName("");
    setBusy(false);
    load();
  ...
  const joinGroup = async (gid: string) => {
    ...
    else { toast.success("تم الانضمام"); load(); }
    setBusy(false);
  };

  const leaveGroup = async () => {
    if (!myGroup) return;
    if (!confirm("هل تريد الخروج من المجموعة؟")) return;
    setBusy(true);
    ...
    if (error) toast.error(error.message);
    else { toast.success("تم الخروج"); load(); }
    setBusy(false);
  };
  ...
  const sendInvite = async (inviteeId: string) => {
    ...
    if (error) toast.error(error.message);
    else toast.success("تم إرسال الدعوة");
    setBusy(false);
    load();
  };
  ...
  const respondInvite = async (inv: Invite, accept: boolean) => {
    ...
    const { error: e2 } = await supabase
      ...
      .eq("id", inv.id);
    if (e2) toast.error(e2.message);
    else toast.success(accept ? "تم قبول الدعوة" : "تم الرفض");
    setBusy(false);
    load();
  };
    ...
    <AppLayout>
      <div className="space-y-6">
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mt-2 flex-wrap text-sm">
              <Badge variant="outline">حد المجموعة: {assignment.max_group_size ?? "—"}</Badge>
        ...
        {!isSelfMode && (
          <Card>
            <CardContent className="py-6">
              <p className="text-sm text-muted-foreground">المسؤول هو من يقوم بتقسيم المجموعات لهذا الطلب. تجد مجموعتك أدناه.</p>
            </CardContent>
          </Card>
        )}
        ...
        {myInvites.length > 0 && (
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-base flex items-center gap-2"><Mail className="h-4 w-4 text-primary" /> دعوات وردت إليك</CardTitle>
            </CardHeader>
            <CardContent className="space-y-2">
              {myInvites.map((inv) => (
                <div key={inv.id} className="flex items-center justify-between gap-2 border rounded-lg p-3 flex-wrap">
                  <div className="text-sm">
                    <span className="font-semibold">{inv.inviter_name}</span> دعاك للانضمام إلى{" "}
                    <span className="font-semibold">{inv.group_name}</span>
                  </div>
                  <div className="flex gap-1">
                    <Button size="sm" className="gap-1" onClick={() => respondInvite(inv, true)} disabled={busy}>
                      <Check className="h-4 w-4" /> قبول
                    </Button>
                    <Button variant="outline" size="sm" className="gap-1" onClick={() => respondInvite(inv, false)} disabled={busy}>
            ...
            {!myGroup && (
              <Dialog open={openCreate} onOpenChange={setOpenCreate}>
                <DialogTrigger asChild>
                  <Button className="gap-2"><Plus className="h-4 w-4" /> إنشاء مجموعة جديدة</Button>
                </DialogTrigger>
                <DialogContent>
                  <DialogHeader><DialogTitle>إنشاء مجموعة</DialogTitle></DialogHeader>
                  <div className="space-y-2">
                    <Label>اسم المجموعة *</Label>
                    <Input value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="فريق المتميزين" />
                  </div>
                  <DialogFooter>
                    <Button onClick={createGroup} disabled={busy}>إنشاء والانضمام</Button>
                  </DialogFooter>
                </DialogContent>
              </Dialog>
            )}
            {myGroup && (
              <>
                <Dialog open={openInvite === myGroup.id} onOpenChange={(v) => setOpenInvite(v ? myGroup.id : null)}>
                  <DialogTrigger asChild>
                    <Button className="gap-2"><UserPlus className="h-4 w-4" /> دعوة طالب لمجموعتي</Button>
                  </DialogTrigger>
                  <DialogContent>
                ...
                <Button variant="outline" className="gap-2 text-destructive hover:text-destructive" onClick={leaveGroup} disabled={busy}>
                  <LogOutIcon className="h-4 w-4" /> الخروج من مجموعتي
                </Button>
              </>
            )}
          </div>
        ...
        {sentInvites.length > 0 && (
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-base">دعوات أرسلتها</CardTitle>
            </CardHeader>
            <CardContent className="space-y-2">
              {sentInvites.map((inv) => {
                ...
                return (
                  <div key={inv.id} className="flex items-center justify-between gap-2 border rounded-lg p-3 flex-wrap">
                    <div className="text-sm">
                      دعوة معلّقة لـ <span className="font-semibold">{invitee?.full_name ?? "طالب"}</span>
                    </div>
          ...
          {groups.length === 0 && (
            <Card className="md:col-span-2">
              <CardContent className="py-10 text-center text-muted-foreground">
                لا توجد مجموعات بعد{canEdit ? " — يمكنك إنشاء واحدة" : ""}
              </CardContent>
            </Card>
          )}
          {groups.map((g) => {
